...
import {
  getProjectAnalysisLayerArtifact,
  processProjectAnalysisLayer,
...
const WaterParticle = () => {
  ...
  const getAnalysisLayerPalette = (layerId: string) => {
  ...
  const applyScalarAnalysisArtifactLayer = (
    ...
    const mesh = currentMesh.current;
    if (!mesh?.geometry || !Array.isArray(artifact?.values)) return false;

    const geometry = mesh.geometry;
    const positionAttribute = geometry.attributes.position;
    if (!positionAttribute) return false;

    if (artifact.values.length !== positionAttribute.count) {
      setAnalysisLayerMessage(
        layer.id,
        "failed",
        `El artefacto tiene ${artifact.values.length} valores para ${positionAttribute.count} vertices.`
      );
      return false;
    }
    ...
    for (let index = 0; index < positionAttribute.count; index += 1) {
      const rawValue = Number(artifact.values[index]);
    ...
    setAnalysisLayerMessage(
      layer.id,
      "ready",
      `${artifact.method ?? "Analisis real"} aplicado (${artifact.values.length} vertices).`,
      manifest
    );
    renderer.current?.render(scene.current!, camera.current!);
    return true;
  ...
  const applyBackendAnalysisLayer = async (
    layer: TerrainAnalysisLayerDefinition,
    message = "Resultado backend recibido y activado.",
    manifest = analysisLayerStates[layer.id]?.manifest
  ) => {
    if (manifest?.source === "backend" && project._id) {
      try {
        const artifact = await getProjectAnalysisLayerArtifact(project._id, layer.id);
        if (applyScalarAnalysisArtifactLayer(layer, artifact, manifest)) {
          return;
        }
      } catch (error) {
        ...
        setAnalysisLayerMessage(
          layer.id,
          "failed",
          `No se pudo pintar el artefacto real. ${detail}`,
          manifest
        );
        return;
      }
    ...
    applyBackendManifestPreviewLayer(layer, message, manifest);
  };
  ...
  const processBackendAnalysisLayer = async (layer: TerrainAnalysisLayerDefinition) => {
    ...
    try {
      ...
      if (manifest.status === "ready") {
        void applyBackendAnalysisLayer(
          layer,
          manifest.source === "backend-stub"
  ...
  const handleActivateAnalysisLayer = (layer: TerrainAnalysisLayerDefinition) => {
    ...
    if (state?.status === "ready" && state.manifest?.source === "backend") {
      void applyBackendAnalysisLayer(layer);
      return;
    }
